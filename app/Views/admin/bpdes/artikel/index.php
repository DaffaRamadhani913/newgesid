...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold gold-text gold-shadow"><i class="bi bi-file-earmark-text me-2"></i> Daftar Artikel</h2>
        <a href="<?= base_url('admin/bpd/buat-artikel') ?>" class="btn btn-gold">
                    ...
                    <tbody>
                        <?php if (!empty($artikels)): ?>
                            <?php foreach ($artikels as $i => $artikel): ?>
                                <tr>
                                    <td><?= $i + 1 ?></td>
                                    <td class="text-start"><?= esc($artikel['judul']) ?></td>
                                    ...
                                    <td>
                                        <?php if (!empty($artikel['gambar'])): ?>
                                            <img src="<?= base_url($artikel['gambar']) ?>" alt="thumbnail"
                                                class="img-thumbnail gold-border" style="max-width: 80px;">
                                        <?php else: ?>
                                            <span class="text-muted fst-italic">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <a href="<?= base_url('admin/bpd/edit-artikel/' . $artikel['id']) ?>"
                                            class="btn btn-warning btn-sm">
                                            <i class="bi bi-pencil-square"></i> Edit
                                        </a>
                                        <form action="<?= base_url('admin/bpd/delete-artikel/' . $artikel['id']) ?>" method="post"
